...
class LinkCapabilityTechnologiesUseCaseTest {
        ...
        void When_AllTechnologiesExist_Expect_LinksToBeSaved() {
                ...
                List<CapabilityTechnology> savedLinks = List.of(
                                CapabilityTechnology.builder().id(LINK_ID).capabilityId(CAPABILITY_ID)
                                                .technologyId(TECHNOLOGY_ID_1).build());

                when(technologyRepository.findMissingIds(command.technologyIds())).thenReturn(Mono.just(List.of()));
                when(capabilityTechnologyRepository.saveAll(any(LinkCapabilityTechnologies.class)))
                ...
                ArgumentCaptor<LinkCapabilityTechnologies> captor = ArgumentCaptor
                                .forClass(LinkCapabilityTechnologies.class);
                verify(capabilityTechnologyRepository).saveAll(captor.capture());
                LinkCapabilityTechnologies saved = captor.getValue();
                assertEquals(CAPABILITY_ID, saved.getCapabilityId().value());
                assertEquals(command.technologyIds(), saved.getTechnologyIds().value());
        }
        ...
        void Expect_TechnologiesNotFoundException_When_SomeTechnologiesDoNotExist() {
                // Arrange
                LinkCapabilityTechnologiesCommand command = new LinkCapabilityTechnologiesCommand(CAPABILITY_ID,
                                List.of(TECHNOLOGY_ID_1, TECHNOLOGY_ID_2, MISSING_TECHNOLOGY_ID));

                when(technologyRepository.findMissingIds(command.technologyIds()))
                                .thenReturn(Mono.just(List.of(MISSING_TECHNOLOGY_ID)));
                ...
                verify(capabilityTechnologyRepository, never()).saveAll(any(LinkCapabilityTechnologies.class));
        }
        ...
        @Test
        void Expect_FieldsValidationException_When_TechnologyIdsBelowMinSize() {
                // Arrange
                LinkCapabilityTechnologiesCommand command = new LinkCapabilityTechnologiesCommand(CAPABILITY_ID,
        ...
        @Test
        void Expect_FieldsValidationException_When_TechnologyIdsExceedsMaxSize() {
                // Arrange
                List<Long> tooManyIds = LongStream.rangeClosed(1, 21).boxed().toList();
                LinkCapabilityTechnologiesCommand command = new LinkCapabilityTechnologiesCommand(CAPABILITY_ID,
                                tooManyIds);
        ...
        void Expect_FieldsValidationException_When_TechnologyIdsHasDuplicates() {
                // Arrange
                LinkCapabilityTechnologiesCommand command = new LinkCapabilityTechnologiesCommand(CAPABILITY_ID,
                                List.of(TECHNOLOGY_ID_1, TECHNOLOGY_ID_1, TECHNOLOGY_ID_2));
